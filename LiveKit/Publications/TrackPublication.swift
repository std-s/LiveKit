...
public class TrackPublication: TrackDelegate, Loggable {
    ...
    internal func set(track newValue: Track?) -> Track? {
        ...
        print("\(String(describing: oldValue)) -> \(String(describing: newValue))")
    ...
    public func track(_ track: Track, didUpdate muted: Bool, shouldSendSignal: Bool) {
        ...
        print("muted: \(muted) shouldSendSignal: \(shouldSendSignal)")
        ...
        sendSignal()
            .recover(on: .sdk) { print("Failed to stop all tracks, error: \($0)") }
            .then(on: .sdk) {
